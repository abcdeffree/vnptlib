 ...
 */
public class UserRequests extends DSpaceObject
{
    ...
    /** The row in the table representing this request */
     ...
     */
    UserRequests(Context context, TableRow row)
    {
        myContext = context;
        myRow = row;

        // Cache ourselves
        context.cache(this, row.getIntColumn("request_id"));
        modified = false;
    ...
    public boolean equals(Object obj)
    {
        ...
        final UserRequests other = (UserRequests) obj;
    ...
    /**
     * Get an UserRequests from the database.
     * 
     * @param context
     *            DSpace context object
     * @param id
     *            ID of the UserRequests
     * 
     * @return the UserRequests format, or null if the ID is invalid.
     */
    public static UserRequests find(Context context, int id) throws SQLException
    {
        // First check the cache
        UserRequests fromCache = (UserRequests) context.fromCache(UserRequests.class, id);
        ...
        TableRow row = DatabaseManager.find(context, "request", id);
        ...
        {
            return new UserRequests(context, row);
        }
    }

    /**
     * Find the request by their email address.
     * 
     * @return UserRequests, or {@code null} if none such exists.
     */
    public static UserRequests findByEmail(Context context, String email)
            throws SQLException, AuthorizeException
    {
        ...
        TableRow row = DatabaseManager.findByUnique(context, "request",
        ...
        {
            // First check the cache
            UserRequests fromCache = (UserRequests) context.fromCache(UserRequests.class, row
                    .getIntColumn("request_id"));
    ...
    /**
     * Find the request by their netid.
     * 
     * @param context
     *            DSpace context
     * @param netid
     *            Network ID
     * 
     * @return corresponding UserRequests, or <code>null</code>
     */
    public static UserRequests findByNetid(Context context, String netid)
            throws SQLException
    {
        if (netid == null)
        {
        ...
        TableRow row = DatabaseManager.findByUnique(context, "request", "netid", netid);
     ...
     *            The search string
     * 
     * @return array of UserRequests objects
     */
    public static UserRequests[] search(Context context, String query)
            throws SQLException
    {
     ...
     * @return array of UserRequests objects
     */
    public static UserRequests[] search(Context context, String query, int offset, int limit) 
    		throws SQLException
	{
		String params = "%"+query.toLowerCase()+"%";
        StringBuffer queryBuf = new StringBuffer();
        queryBuf.append("SELECT * FROM request WHERE request_id = ? OR ");
        ...
        // When checking against the request-id, make sure the query can be made into a number
		Integer int_param;
		try {
		...
		try
        {
            ...
            UserRequests[] epeople = new UserRequests[epeopleRows.size()];
            ...
            for (int i = 0; i < epeopleRows.size(); i++)
            {
                ...
                {
                    epeople[i] = new UserRequests(context, row);
                }
            }
     ...
     * of creating the UserRequests objects to store the results.
     * 
     * @param context
     *            DSpace context
     * @param query
    	...
    	throws SQLException
	{
		...
		TableRow row = DatabaseManager.querySingle(context,
		        "SELECT count(*) as epcount FROM request WHERE request_id = ? OR " +
     ...
     * @return array of UserRequests objects
     */
    public static UserRequests[] findAll(Context context, int sortField)
            throws SQLException
    {
        String s;
        ...
        TableRowIterator rows = DatabaseManager.query(context, 
                "SELECT * FROM request ORDER BY "+s);
     ...
     *            DSpace context object
     */
    public static UserRequests create(Context context) throws SQLException,
            AuthorizeException
    {
        // authorized?
        if (!AuthorizeManager.isAdmin(context))
        {
            throw new AuthorizeException(
                    "You must be an admin to create an UserRequests");
        }

        // Create a table row
        TableRow row = DatabaseManager.create(context, "user_requests");

        UserRequests e = new UserRequests(context, row);

        log.info(LogManager.getHeader(context, "create_request", "request_id="
                + e.getID()));

        context.addEvent(new Event(Event.CREATE, Constants.USER_REQUESTS, e.getID(), null));
...
//            UserRequestsDeletionException
    {
        // authorized?
        if (!AuthorizeManager.isAdmin(myContext))
        {
            throw new AuthorizeException(
                    "You must be an admin to delete an UserRequests");
        }

        // check for presence of request in tables that
        // have constraints on request_id
        List<String> constraintList = getDeleteConstraints();

        // if request exists in tables that have constraints
        // on request, throw an exception
        if (constraintList.size() > 0)
        {
//            throw new UserRequestsDeletionException(constraintList);
        }

        myContext.addEvent(new Event(Event.DELETE, Constants.USER_REQUESTS, getID(), getEmail()));
        ...
        DatabaseManager.updateQuery(myContext,
                "DELETE FROM UserRequestsGroup2UserRequests WHERE request_id= ? ",
                getID());

        // Remove any subscriptions
        DatabaseManager.updateQuery(myContext,
                "DELETE FROM subscription WHERE request_id= ? ",
                getID());

        // Remove ourself
        DatabaseManager.delete(myContext, myRow);

        log.info(LogManager.getHeader(myContext, "delete_request",
                "request_id=" + getID()));
    }
    ...
    public int getID()
    {
        return myRow.getIntColumn("request_id");
    }
     ...
     */
     ...
     /**
     * Set the UserRequests's language.  Value is expected to be a Unix/POSIX
    ...
    /**
     * Set the UserRequests's password
     * 
     * @param s
     *            the new email
     */
    ...
    /**
     * Set the UserRequests's password hash
     * 
     * @param s
    ...
    /**
     * Return the UserRequests's password hash
    ...
    /**
     * Update the UserRequests
     */
    public void update() throws SQLException, AuthorizeException
    {
        // Check authorisation - if you're not the request
        ...
        log.info(LogManager.getHeader(myContext, "update_request",
                "request_id=" + getID()));
    ...
    /**
     * Check for presence of UserRequests in tables that have constraints on
     * UserRequestss. Called by delete() to determine whether the request can
     * actually be deleted.
     * 
     * An UserRequests cannot be deleted if it exists in the item, workflowitem, or
     * tasklistitem tables.
     * 
     * @return List of tables that contain a reference to the request.
     */
    public List<String> getDeleteConstraints() throws SQLException
    {
        List<String> tableList = new ArrayList<String>();

        // check for request in item table
        ...
        // explaining to the user why the request cannot be deleted.
        return tableList;
    }

    private void getXMLWorkflowConstraints(List<String> tableList) throws SQLException {
         TableRowIterator tri;
        // check for request in claimtask table
        ...
        // check for request in pooltask table
        tri = DatabaseManager.queryTable(myContext, "cwf_pooltask",
                "SELECT * from cwf_pooltask where request_id= ? ",
                getID());
        ...
        // check for request in workflowitemrole table
        tri = DatabaseManager.queryTable(myContext, "cwf_workflowitemrole",
                "SELECT * from cwf_workflowitemrole where request_id= ? ",
                getID());
    ...
    private void getOriginalWorkflowConstraints(List<String> tableList) throws SQLException {
        TableRowIterator tri;
        // check for request in workflowitem table
        ...
        // check for request in tasklistitem table
        tri = DatabaseManager.query(myContext, 
                "SELECT * from tasklistitem where request_id= ? ",
                getID());
