...
<dspace:layout locbar="commLink" title="<%= name%>" feedData="<%= feedData%>">
    <div class="div_home_content">
        <h3><img  class="nav_home_h3" src="<%= request.getContextPath()%>/image/collection.png"/><fmt:message key="jsp.collection-home.heading1"/>: <%= name%>
            <%
        ...
        <table width="100%" align="center" cellspacing="10">
            <tr>
                <td>
                    <%-- HACK:  used for Netscape 4.x, which doesn't accept align="center"
        ...
        <dspace:sidebar>
            ...
            <div class="div_box_rightbar">
                <h3><img  class="nav_home_h3" src="<%= request.getContextPath()%>/image/document_new.png"/><fmt:message key="jsp.collection-home.recentsub"/></h3>
                    <%
                        if (rs != null) {
                            Item[] items = rs.getRecentSubmissions();
                            for (int i = 0; i < items.length; i++) {
                    ...
                    %><p class="recentItem"><img  class="a_recentitem" src="<%= request.getContextPath()%>/image/transparent.gif"/><a href="<%= request.getContextPath()%>/handle/<%= items[i].getHandle()%>"><%= displayTitle%></a></p><%
                            }
                        }
                    %>
            </div>
            ...
            %>
                ...
                %>
                <a href="<%= request.getContextPath()%>/feed/<%= fmts[j]%>/<%= collection.getHandle()%>"><img src="<%= request.getContextPath()%>/image/<%= icon%>" alt="RSS Feed" width="<%= width%>" height="15" vspace="3" /></a>
                    <%
                        }
                    %>
