...
public class ItemTag extends TagSupport {
    ...
    private static final String HANDLE_DEFAULT_BASEURL = "http://lib.vnpt.vn/";
            ...
            if (values.length > 0) {
                ...
                for (int j = 0; j < values.length; j++) {
                    if (values[j] != null && values[j].value != null) {
                        if ("embargo".equalsIgnoreCase(qualifier)) {
                            DCDate ed = new DCDate(values[j].value);
                            Date embargo_date = ed.toDate();
                            Date today = new Date();
                            if (today.compareTo(embargo_date) < 0) {
                                //embargo current
                                embargo = true;
                            }
                        }
    ...
    private void renderFull() throws IOException, SQLException {
        ...
        for (int i = 0; i < values.length; i++) {
            if (!MetadataExposure.isHidden(context, values[i].schema, values[i].element, values[i].qualifier)) {
                ...
                if ("embargo".equalsIgnoreCase(values[i].qualifier)) {
                    DCDate ed = new DCDate(values[i].value);
                    Date embargo_date = ed.toDate();
                    Date today = new Date();
                    if (today.compareTo(embargo_date) < 0) {
                        //embargo current
     ...
     */
    private void listBitstreams(boolean embargo) throws IOException {
        JspWriter out = pageContext.getOut();
        HttpServletRequest request = (HttpServletRequest) pageContext.getRequest();
        ...
        try {
            ...
            if (!filesExist) {
                ...
                out.println("<th id=\"t3\" class=\"standard\">"
                        ...
                        + "</th><th><a href=\"#div_download\" class=\"download\">Download</a></th></tr>");
...
//                                out.print(bsLink);
                                ...
                                String bsUrl = ConfigurationManager.getProperty("dspace.baseUrl") + request.getContextPath() + "/bitstream/"
                                            + item.getHandle()
                                            + "/" + bitstreams[k].getSequenceID()
...
//                                                    }
                                ...
                                if (embargo) {
                                    ...
                                    } else if (bsUrl.indexOf(".mpg") > -1 || bsUrl.indexOf(".mpeg") > -1) {
                                        //
                                        out.print("<a href=\"#div_preview\" class=\"mpg_preview\" rel=\"" + bsUrl + "\">Xem</a>");
                                    } else {
                                        out.print("<a href=\"" + bsUrl + "\" class=\"viewdown\" rel=\"" + bsUrl + "\">Download</a>");
                                    }
     ...
     * <code>null</code> if the field is not a browse field (look for <cod>webui.browse.link.<n></code>
     * in dspace.cfg)
     *
     * @param field
