...
public class RegisterServlet extends DSpaceServlet
{
     ...
     * @thuclh
     */
    private boolean verifyEmailLocal(String email){
        String allow_mail_server = ConfigurationManager.getProperty("allow.mail.server");
        String[] allow_mail_servers = allow_mail_server.split(",");
        if(allow_mail_servers.length > 0){
            String mail_server = "";
            for (int i = 0; i < allow_mail_servers.length; i++) {
                mail_server = allow_mail_servers[i];
                if(email.indexOf(mail_server) > -1){
                    return false;
                }
            }
            return true;
