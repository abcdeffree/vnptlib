...
public class RecentSubmissionsManager
{
	 ...
	 * @param dso	DSpaceObject: Community or Collection
         ...
         * @param count	Number of item is displayed
         * @param sortOption	sort option
	 * @return		The recently submitted items
	 * @throws RecentSubmissionsException
	 */
	public RecentSubmissions getRecentSubmissions(DSpaceObject dso,String source, String count, String sortOption)
		throws RecentSubmissionsException
	{
		try
		{
			// get our configuration
                        //source = ConfigurationManager.getProperty("recent.submissions.sort-option");
			if(count == null){
                            count = ConfigurationManager.getProperty("recent.submissions.count");
                        }
			// prep our engine and scope
			BrowseEngine be = new BrowseEngine(context);
			BrowserScope bs = new BrowserScope(context);
	 ...
	 * @throws RecentSubmissionsException
	 */
	public String[][] getRecentAuthorSubmissions(DSpaceObject dso,String type, String count, String sortColumn, String sortOption)
		throws RecentSubmissionsException
	{
		try
		{
            ...
            for (SortOption so : SortOption.getSortOptions())
            {
                if (so.getName().equals(sortColumn))
                {
                    bs.setSortBy(so.getNumber());
                }
            }
			...
			String[][] rs = results.getStringResults();
