...
public class Item extends DSpaceObject
{
     ...
     * @throws SQLException
     */
    public static ItemIterator findSubjects(Context context) throws SQLException
    {
        String myQuery = "SELECT * FROM item WHERE in_archive='1'";
...
//               Remove from indices
     ...
     */
    public Comment[] getComments() throws SQLException
    {
        ...
        TableRowIterator tri = DatabaseManager.queryTable(
        	ourContext,"comment",
            "SELECT comment.* FROM comment WHERE " +
            "comment.item_id= ? ORDER BY comment.comment_id desc",
            getID());
        ...
        try
        {
            while (tri.hasNext())
            {
                ...
                // First check the cache
                Comment fromCache = (Comment) ourContext.fromCache(
                        Comment.class, row.getIntColumn("comment_id"));

                if (fromCache != null)
                {
                    comments.add(fromCache);
                }
                else
                {
                    comments.add(new Comment(ourContext, row));
                }
            }
        }
        finally
        ...
        commentArray = (Comment[]) comments.toArray(commentArray);
